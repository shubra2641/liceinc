 ...
 *
 * This service provides comprehensive webhook management with retry logic,
 * signature verification, event filtering, and advanced monitoring capabilities.
 *
 * Features:
 ...
 * - Bulk operations and delivery reports
...
class AdvancedWebhookService
{
     ...
     *
     * Processes webhook events by sending them to all active webhooks that
     ...
     *
     * Handles the complete webhook delivery process including data preparation,
     ...
     *
     * Creates a secure webhook payload with proper structure, timestamps,
     ...
     *
     * Executes HTTP POST request to webhook URL with comprehensive headers,
     ...
     *
     * Records detailed information about webhook delivery attempts including
     ...
     *
     * Updates webhook statistics including attempt counts, success/failure
     ...
     *
     * Implements intelligent retry logic with exponential backoff delays
     ...
     *
     * Calculates the current attempt number based on recent webhook logs
     ...
     *
     * Handles webhook failure after exhausting all retry attempts by logging
     ...
     *
     * @example
     * $isValid = $webhookService->verifyWebhookSignature(
     ...
     *
     * Sends a test webhook to verify endpoint connectivity, response handling,
     ...
     * $result = $webhookService->testWebhook($webhook);
     * if ($result['success']) {
     *     echo "Webhook test successful - Response time: " . $result['response_time'] . "s";
     * } else {
     ...
     *
     * Analyzes webhook delivery performance over a specified time period,
     ...
     *
     * Removes webhook logs older than the specified number of days to maintain
     ...
     *
     * @example
     * $deletedCount = $webhookService->cleanupOldLogs(30);
     * echo "Cleaned up $deletedCount old webhook logs";
     */
     ...
     *
     * Analyzes recent webhook performance to determine health status based on
     ...
     *
     * @example
     * $health = $webhookService->getWebhookHealth($webhook);
     ...
     *
     * Updates multiple webhooks simultaneously for improved performance
     ...
     *
     * Creates a detailed delivery report for all webhooks over a specified
     ...
     *
     * @example
     * $report = $webhookService->getDeliveryReport(30);
     ...
     *
     * Calculates the execution time for webhook processing to enable
     * performance monitoring and optimization. Uses high-precision timing
     * for accurate measurements.
     *
     * @return float Execution time in seconds with microsecond precision
     */
     ...
     *
     * Queues webhook events for asynchronous processing to improve application
     ...
     *
     * Performs thorough validation of webhook URLs including format validation,
     ...
     *
     * @example
     * $validation = $webhookService->validateWebhookUrl('https://example.com/webhook');
     * if (!$validation['valid']) {
