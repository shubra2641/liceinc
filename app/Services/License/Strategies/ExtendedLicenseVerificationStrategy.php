...
/**
 * Extended License Verification Strategy
 * 
 * Handles verification for extended licenses with additional features
 */
class ExtendedLicenseVerificationStrategy implements LicenseVerificationStrategy
{
    /**
     * Verify license
     */
    public function verify(License $license, array $data): array
    {
        ...
        if (!$this->hasValidUsage($license)) {
            return $this->createErrorResponse('USAGE_EXCEEDED', 'License usage limit exceeded');
        }
    ...
    {
        return in_array($licenseType, ['extended', 'premium', 'enterprise']);
    }
    ...
    /**
     * Check if license has valid usage
     */
    private function hasValidUsage(License $license): bool
    {
        if (!$license->usage_limit) {
            return true;
        }

        $currentUsage = $license->activation_count ?? 0;
        return $currentUsage < $license->usage_limit;
    }
    ...
    private function createSuccessResponse(License $license, array $data): array
    {
        $response = [
            ...
            'current_usage' => $license->activation_count ?? 0,
        ];
