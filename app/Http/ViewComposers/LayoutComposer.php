 ...
 * This view composer binds layout data to views including site settings, * SEO configuration, and other global layout variables with comprehensive * security measures and performance optimization. *
 * Features: * - Site settings and configuration binding * - SEO settings management and validation * - Performance optimization with caching * - Comprehensive error handling and logging * - Enhanced security measures for data binding * - Input validation and sanitization * - Clean and maintainable code structure *
 *
 * @example * // Register in AppServiceProvider * View::composer('layouts.*', LayoutComposer::class);
 *
 * // Use in Blade template * {{ $siteName }} - {{ $siteSeoTitle }} */
class LayoutComposer
{
    ...
    /**   * Bind data to the view with enhanced security and performance. *   * Composes layout data including site settings, SEO configuration, * and other global variables with proper error handling and caching. *   * @param View $view The view instance to bind data to *   * @throws \Exception When data binding fails *   * @example * $composer = new LayoutComposer(); * $composer->compose($view); */
    ...
    /**   * Get layout data with caching and error handling. *   * Retrieves all layout-related data including site settings and SEO * configuration with performance optimization through caching. *   * @return array<string, mixed> Array of layout data *   * @throws \Exception When data retrieval fails */
    ...
    /**   * Get site name with validation and fallback. *   * @return string The site name */
    private function getSiteName(): string
    {
        $siteName = Setting::get('site_name', config('app.name', 'Laravel'));
        return $this->sanitizeOutput(is_string($siteName) ? $siteName : '');
    }
    /**   * Get site logo with validation. *   * @return string|null The site logo URL or null */
    ...
    /**   * Get site SEO title with validation. *   * @return string|null The site SEO title or null */
    ...
    /**   * Get site SEO description with validation. *   * @return string|null The site SEO description or null */
    ...
    /**   * Get Open Graph image with validation. *   * @return string|null The Open Graph image URL or null */
    ...
    /**   * Get Knowledge Base SEO title with validation. *   * @return string|null The KB SEO title or null */
    ...
    /**   * Get Knowledge Base SEO description with validation. *   * @return string|null The KB SEO description or null */
    ...
    /**   * Get Tickets SEO title with validation. *   * @return string|null The Tickets SEO title or null */
    ...
    /**   * Get Tickets SEO description with validation. *   * @return string|null The Tickets SEO description or null */
    ...
    /**   * Get available languages with metadata. *   * @return array<int, array<string, string>> The available languages */
    private function getAvailableLanguages(): array
    {
        return LanguageController::getAvailableLanguagesWithMetadata();
    }
    /**   * Get current locale. *   * @return string The current locale */
    private function getCurrentLocale(): string
    {
        return app()->getLocale();
    }
    /**   * Get current language metadata. *   * @return array<string, mixed>|null The current language metadata */
    ...
    /**   * Get preloader settings. *   * @return array<string, mixed> The preloader settings */
    ...
    /**   * Get fallback data for error scenarios. *   * Provides minimal fallback data when the main data retrieval fails * to prevent view rendering errors. *   * @return array<string, mixed> Array of fallback data */
    ...
    /**   * Clear layout composer cache. *   * Clears the cached layout data to force refresh on next request. */
    ...
    /**   * Sanitize output to prevent XSS attacks. *   * @param string $output The output to sanitize *   * @return string The sanitized output */
