...
(function(global){
    ...
    function basicSanitize(html){
        ...
        return html.replace(/<([^\s>\/]+)([^>]*)>/g,function(match,tag,attrs){
            tag = tag.toLowerCase();
            const allowed = ['b','i','strong','em','u','p','br','ul','ol','li','a','span','div'];
            ...
            if(tag === 'a'){
                ...
                if(hrefMatch){
                    ...
                    if(/^javascript:/i.test(url)) return '<a>'; // strip dangerous
