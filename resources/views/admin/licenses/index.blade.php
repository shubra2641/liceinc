...
<div class="dashboard-grid dashboard-grid-4 stats-grid-enhanced">
    ...
    <div class="stats-card stats-card-primary animate-slide-up">
        ...
        <div class="stats-card-content">
            ...
            <div class="stats-card-body">
                <div class="stats-card-value">{{ $licenses->total() }}</div>
                ...
                <div class="stats-card-trend positive">
                    ...
                    <span>{{ trans('app.all_issued_licenses') }}</span>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="stats-card stats-card-success animate-slide-up animate-delay-200">
        ...
        <div class="stats-card-content">
            ...
            <div class="stats-card-body">
                <div class="stats-card-value">{{ $licenses->where('status', 'active')->count() }}</div>
    ...
    <div class="stats-card stats-card-warning animate-slide-up animate-delay-300">
        ...
        <div class="stats-card-content">
            ...
            <div class="stats-card-body">
                <div class="stats-card-value">{{ $licenses->filter(function($license) { return $license->license_expires_at && $license->license_expires_at->diffInDays() <= 30; })->count() }}</div>
                <div class="stats-card-label">{{ trans('app.Expiring Soon') }}</div>
                <div class="stats-card-trend negative">
                    <i class="stats-trend-icon negative"></i>
                    <span>{{ trans('app.within_30_days') }}</span>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="stats-card stats-card-danger animate-slide-up animate-delay-400">
        ...
        <div class="stats-card-content">
            ...
            <div class="stats-card-body">
                <div class="stats-card-value">{{ $licenses->filter(function($license) { return $license->license_expires_at && $license->license_expires_at->isPast(); })->count() }}</div>
            ...
            <div class="card-body p-4">
                <div class="d-flex align-items-center">
                    ...
                    <div class="flex-grow-1">
                        <p class="text-muted small mb-1 fw-medium">{{ trans('app.Total Domains') }}</p>
                        <h3 class="fw-bold mb-1 text-dark">{{ $licenses->sum(function($license) { return $license->active_domains_count ?? 0; }) }}</h3>
                        <small class="text-info fw-medium">{{ trans('app.Active domains') }}</small>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="card border-0 shadow-sm stats-card">
            <div class="card-body p-4">
                <div class="d-flex align-items-center">
                    ...
                    <div class="flex-grow-1">
                        <p class="text-muted small mb-1 fw-medium">{{ trans('app.Domain Limit Reached') }}</p>
                        <h3 class="fw-bold mb-1 text-dark">{{ $licenses->filter(function($license) { return $license->hasReachedDomainLimit(); })->count() }}</h3>
                        <small class="text-warning fw-medium">{{ trans('app.Licenses at limit') }}</small>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="card border-0 shadow-sm stats-card">
            <div class="card-body p-4">
                <div class="d-flex align-items-center">
                    ...
                    <div class="flex-grow-1">
                        <p class="text-muted small mb-1 fw-medium">{{ trans('app.Single Domain Licenses') }}</p>
                        <h3 class="fw-bold mb-1 text-dark">{{ $licenses->where('license_type', 'single')->count() }}</h3>
                        <small class="text-success fw-medium">{{ trans('app.One domain only') }}</small>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="card border-0 shadow-sm stats-card">
            <div class="card-body p-4">
                <div class="d-flex align-items-center">
                    ...
                    <div class="flex-grow-1">
                        <p class="text-muted small mb-1 fw-medium">{{ trans('app.Multi Domain Licenses') }}</p>
                        <h3 class="fw-bold mb-1 text-dark">{{ $licenses->whereIn('license_type', ['multi', 'developer', 'extended'])->count() }}</h3>
                        <small class="text-primary fw-medium">{{ trans('app.Multiple domains') }}</small>
                    </div>
                </div>
            </div>
        </div>
                        ...
                        <td class="text-center">
                            <div class="d-flex flex-column align-items-center">
                                <span class="badge bg-info mb-1">
                                    <i class="fas fa-globe me-1"></i>{{ $license->active_domains_count ?? 0 }}/{{ $license->max_domains ?? 1 }}
                                </span>
                                <small class="text-muted">{{ trans('app.Domains') }}</small>
                            </div>
                        </td>
                        <td class="text-center">
