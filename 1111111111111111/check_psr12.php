...
class check_psr12
{
    ...
    private function checkLine($line, $lineNumber, $allLines = [], $prevLineIndex = -1)
    {
        ...
        if (! preg_match('/^\s*(\/\/|#|\/\*|\*)/', $trimmedLine) && // Skip comments
            ...
            ! preg_match('/(if|elseif|while|for|foreach|switch|catch)\s+\(/', $trimmedLine) &&
            ! preg_match('/\w+\s+\([^)]*\)\s*=>/', $trimmedLine)) { // Skip arrow functions
            $issues[] = [
                'line' => $lineNumber,
                'type' => 'Spacing',
        ...
        // 10. Check for proper brace placement for control structures
        if (preg_match('/^(if|elseif|else|while|for|foreach|switch|try|catch|finally)\s*\(/', $trimmedLine)) {
            if (! preg_match('/\{$/', $trimmedLine) && ! preg_match('/\)\s*\{$/', $trimmedLine)) {
                // Check next line for opening brace
                $nextLineIndex = $lineNumber; // 1-based to 0-based
                if (isset($allLines[$nextLineIndex]) && ! preg_match('/^\s*\{/', $allLines[$nextLineIndex])) {
                    $issues[] = [
                        'line' => $lineNumber,
        ...
        if ((preg_match('/([^=!<>\s])=\s*([^=\s>])/', $trimmedLine) || // No space before =
